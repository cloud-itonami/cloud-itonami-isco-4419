(ns clericalsupport.actor
  "ClericalSupportWorkersActor — the ISCO-08 4419 community clericalsupport actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one clericalsupport operation request
  ...
  The unconditional invariant: the ClericalSupportWorkersAdvisor can never
  directly commit a record the ClericalSupportWorkersGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [clericalsupport.advisor :as advisor]
            [clericalsupport.governor :as governor]
            [clericalsupport.store :as store]))

(defn build-graph
  "Build a compiled ClericalSupportWorkersActor graph. `store` implements
  `clericalsupport.store/Store`. `advisor` implements
  `clericalsupport.advisor/Advisor` (defaults to `mock-advisor`).
